...
`, g.Round.RoundNumber)
	...
	playerChoice = strings.ToLower(strings.Replace(playerChoice, "\n", "", -1))
	...
	g.DisplayChan <- fmt.Sprintf("Player chose %s", strings.Title(playerChoice))
	<-g.DisplayChan
...
`, g.Round.PlayerScore, g.Round.ComputerScore)
	...
	if g.Round.PlayerScore > g.Round.ComputerScore {
		g.DisplayChan <- "Player WINS the match!"
		// *** changed here
		<-g.DisplayChan
	} else {
		g.DisplayChan <- "Computer WINS the match!"
		// *** changed here
		<-g.DisplayChan
	}
